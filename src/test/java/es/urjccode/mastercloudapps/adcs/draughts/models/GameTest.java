...
public class GameTest {
	...
	private Error advance(Coordinate[][] coordinates) {
		...
		for (int i = 0; i < coordinates.length; i++) {
			assertNull(error);
			error = game.isValidMove(coordinates[i][0], coordinates[i][1]);
			if (error == null) {
	...
	@Test
	public void testGivenGameWhenGetPiecesByColorThenOk() {
		List<Piece> pieces = game.getPieces(Color.WHITE);
		for (int i = 0; i < pieces.size(); i++) {
			assertEquals(pieces.get(i).getColor(), Color.WHITE);
		}
	}
